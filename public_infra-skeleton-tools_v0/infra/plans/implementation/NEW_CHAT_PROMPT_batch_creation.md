...
## Conditional batch-creation ANX hooks

Keep this batch-creation prompt clean. Do not embed full conditional annex tables here. Instead, use compact `BATCH_CREATION_ANX*.md` hook files.

When generating a batch, read the hook registry after the base planning files and before deciding whether to ask the user for a full `SPEC_Layer...ANX...md` annex.
...
BATCH_CREATION_ANX03_mres_latex_architecture.md
```
...
2. Check whether the selected BATCH_NUMBER or real-organ batch is listed as required or recommended.
...
5. If the full SPEC annex is provided, read it after the relevant layer SPEC and before writing the generated batch files.
6. Put only selected-batch-relevant points into PROJECT_CACHE.md, SPEC.md, and RUN_INSTRUCTIONS.md.
7. Do not paste the full hook or full SPEC annex into generated batch files.
```

The hook files do not change the corrected 01-24 skeleton batch slicing. They only decide which extra full annex files should be requested for specific batches and how those annexes should be consumed.
...
10. Conditional batch-creation hook files, especially `BATCH_CREATION_ANX01_spectral_operator_dsl_bridge.md` and `BATCH_CREATION_ANX02_art_nca_core_architecture.md`; then any full `SPEC_Layer...ANX...md` files those hooks require or recommend for the selected batch
