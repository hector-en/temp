# Organ Track — Codex Operator Execution

Use this only after ChatGPT has generated one concrete config-integration Codex batch from `INTEGRATION_MANIFEST.md`.

## Purpose

Execute one controlled vmuser/operator config integration batch for completed real-organ evidence. Do not infer new scope.
...
Prefer additive, narrow changes: wrapper, health check, lv profile entry, role alias, workflow registration, dry-run launcher, guarded-live launcher, or status command.

## What not to do

```text
do not edit project organ/science code
do not change preserved skeleton contracts
...
Only run organ smoke commands if SPEC says they are local, safe, and dry-run/guarded.
